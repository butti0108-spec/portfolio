# 28 only: mat+breath (no gym props), intimate forest for night
...
$tmpRoot = Join-Path $env:TEMP ("photo-28b-" + [guid]::NewGuid().ToString("n"))
New-Item -ItemType Directory -Force -Path $tmpRoot | Out-Null
$used = New-Object "System.Collections.Generic.HashSet[string]"
$v = [DateTimeOffset]::UtcNow.ToUnixTimeMilliseconds()
$bad = "dumbbell|dumbbells|weight|pilates|ring|nike|under armour|acro|handstand|peacock|mayurasana|gym|fitness|workout|skunk|insect|bee|butterfly|beetle"
...
function Pick([string[]]$qs) {
  foreach ($q in $qs) {
    ...
    foreach ($item in @($r.results)) {
      ...
      $blob = ([string]$item.title + " " + [string]$item.tags + " " + [string]$item.foreign_landing_url).ToLowerInvariant()
...
function Apply-Slot([string]$file, [string]$key, [string[]]$qs, [switch]$Sync) {
  $dir = Folder "28"
  Write-Host "== 28 $file =="
  $hit = Pick $qs
  if (-not $hit) { Write-Host "FAIL 28 $file"; return }
  ...
  Write-Host "OK 28 $file $($hit.Query) :: $($hit.Url)"
  Add-Content (Join-Path $dir.FullName "SOURCES-photo-wv.md") "| 28 | $file | $($hit.Query) | $($hit.Url) | 28breath |" -Encoding UTF8
}

Apply-Slot "hero.jpg" "hero_image" @(
  "hatha yoga seated",
  "pranayama yoga",
  "yoga lotus pose mat",
  "yoga child pose",
  "yoga warrior pose outdoor",
  "woman meditating yoga mat",
  "yoga studio empty mats"
)
Apply-Slot "about-01.jpg" "about_image_1" @(
  "yoga class mats floor",
  "hatha yoga indoor",
  "yoga seated meditation",
  "person yoga mat breathing",
  "yoga studio wooden floor practice"
) -Sync
Apply-Slot "work-02.jpg" "work_2_image" @(
  "forest path trees green dense",
  "inside forest trees moss",
  "woodland trail green canopy",
  "deep forest trees calm",
  "forest interior green soft light"
)
